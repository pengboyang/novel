      ...
      getUser() {
        ...
        this.$http({
          ...
          if (data.code == 1) {
            ...
            location.href = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx30e74a0a5ca3c0bd&redirect_uri=http%3a%2f%2fs.55duanzi.com%2fnovel%2fdist%2findex.html%23%2fnovel%2fmineList%3fid%3d4&response_type=code&scope=snsapi_userinfo&state=user#wechat_redirect';
          } else if (data.code == 4) {
            this.login(data=> {
              if(data=='success'){
                this.getUser();
